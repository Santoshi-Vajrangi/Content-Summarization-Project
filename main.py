...
from youtube.youtube import PalmYoutube
...
# Streamlit app main function for YouTube Comment Summarizer
def youtube_comment_summarizer():
    
    st.title("YouTube Comment Summarizer")

    video_url = st.text_input("Enter YouTube Video URL:")

    summaryGenerator = PalmYoutube(video_url)
    
    if st.button("Generate Summary"):
        summary = summaryGenerator.scrape_all_with_replies(video_url)
        st.success(summary)


# Streamlit app main function for News Article Scraper and Summarizer
def news_article_scraper():
    st.title("News Article Scraper and Summarizer")

    # Get the link to the news article from the user
    links = st.text_input(
        "Enter a list of links to the news articles, separated by commas:"
    )
...
# Streamlit app main
def main():
    st.set_page_config(page_title="Multi-Function App", page_icon="📰🎥", layout="wide")

    app_mode = st.selectbox(
        "Select the application", ["News Article Scraper", "YouTube Comment Summarizer"]
    )

    if app_mode == "News Article Scraper":
        news_article_scraper()
    elif app_mode == "YouTube Comment Summarizer":
        youtube_comment_summarizer()
